...
## Vue2 的生命周期都有哪些？请说一下整体流程

::: details 参考答案
| 生命周期 | 描述 | 使用场景 |
| ------------ | ------------------------------------------------------------ | -------------------------------------------------------- |
| **beforeCreate** | 组件实例被创建之初 | 执⾏时组件实例还未创建，通常⽤于插件开发中执⾏⼀些初始化任务 |
| **created** | 组件实例已经完全创建 | 组件初始化完毕，各种数据可以使⽤，常⽤于异步数据获取 |
| **beforeMount** | 组件挂载之前 | 未执⾏渲染、更新，dom 未创建 |
| **mounted** | 组件挂载到实例上去之后 | 初始化结束，dom 已创建，可⽤于获取访问数据和 dom 元素 |
| **beforeUpdate** | 性能较好，避免重复计算 | 更新前，可⽤于获取更新前各种状态 |
| **updated** | 组件数据更新之后 | 更新后，所有状态已是最新 |
| **beforeDestroy** | 组件实例销毁之前 | 销毁前，可⽤于⼀些定时器或订阅的取消 |
| **destroyed** | 组件实例销毁之后 | 组件已销毁，作⽤同上 |
...
- 当数据发生变化时，视图会自动更新
- 当用户操作视图（如输入内容）时，数据也会自动更新

**Vue 中实现双向绑定的方式**

- 在 Vue 中，主要通过 `v-model` 指令实现双向绑定

**Vue 中双向绑定框架的组成**

- 数据层（`Model`）：应⽤的数据及业务逻辑
- 视图层（`View`）：应⽤的展示效果，各类 UI 组件
- 业务逻辑层（`ViewModel`）：框架封装的核⼼，它负责将数据与视图关联起来

**Vue 的双向绑定基于以下几个核心机制**

- 数据劫持/代理：使用 `Object.defineProperty`（Vue 2）或 `Proxy`（Vue 3）来监听数据变化

- 发布-订阅模式：通过 `Dep` 和 `Watcher` 实现依赖收集和通知更新

- 指令解析：模板编译时解析 `v-model` 等指令
  :::

  ## vue2 组件 data 属性是一个函数而不是一个对象？

  ::: details 参考答案
  Vue 组件 data 必须是函数，以保证实例复用时的数据独立性，避免多实例共享同一对象导致状态污染。
  :::
...
- Vue 3 组件支持多个 `v-model` 绑定和自定义属性名，父子之间的双向绑定更加灵活;Vue 2 使用 value + input 事件;
- listeners（透传）：
  💡 Vue 2 :`$attrs`接收父组件传递的非 props 属性（未在 props 中声明的属性）,`$listeners`接收父组件传递的所有事件（包括 v-on 绑定的事件）。
  💡 Vue 3 :$listeners 被移除，所有事件也合并到 $attrs 中。
...
关联文章：[5min 带你快速回顾、学习 VueRouter 的使用！](https://juejin.cn/post/7359084604663840820)
:::
